...
class GitHubRepositoryStatistics : JFrame("GitHub Repository Statistics"), CoroutineScope {
    ...
    private fun parseRepositoryUrl(repositoryUrl: String): Pair<String, String> =
            repositoryUrl.split("/").takeLast(2).zipWithNext()[0]
    ...
    private fun updateResults(
            results: Map<String, UserStatistics>,
            startTime: Long,
    ...
    private val username = JTextField(TEXT_FIELD_WIDTH)
    private val password = JPasswordField(TEXT_FIELD_WIDTH)
    private val repositoryUrl = JTextField(TEXT_FIELD_WIDTH)
    ...
    private val resultsScroll = JScrollPane(results).apply { preferredSize = Dimension(200, 600) }
...
suspend fun loadResults(
    ...
    val commits =
            service.getCommits(req.owner, req.repository).body()?.filter {
                it.author?.type != "Bot" && it.author != null
            }
                    ?: emptyList()

    val channel = Channel<Pair<String, CommitDetails>>()

    val all = Collections.synchronizedMap(mutableMapOf<String, UserStatistics>())

    commits.map {
        async {
            channel.send(
                    Pair(it.author!!.login, it.commit),
            )
        }
    }
    ...
    launch {
        repeat(commits.size) {
            ...
            val fileChanges =
                    service.getChanges(req.owner, req.repository, commits[it].sha).body()?.files
                            ?: listOf()

            val countChanges = fileChanges.fold(0) { acc, change -> acc + change.changes }

            val files = fileChanges.map { file -> file.filename }.toSet()

            val statistics =
                    UserStatistics(
                            (all[received.first]?.commits?.plus(1)) ?: 1,
                            all[received.first]?.files?.union(files) ?: files,
                            all[received.first]?.changes?.plus(countChanges) ?: countChanges,
                    )

            all[received.first] = statistics

            updateResults(all, it == commits.lastIndex)
        }
    }
}
